...
export async function PUT(
  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      priceStr = json.price !== undefined ? String(json.price) : "";
      categoryIdStr = json.categoryId !== undefined ? String(json.categoryId) : "";
      statusStr = json.status || "active";
    } else {
      const formData = await req.formData();
      title = formData.get("title") as string;
      description = formData.get("description") as string;
      priceStr = formData.get("price") as string;
      categoryIdStr = formData.get("categoryId") as string;
      statusStr = (formData.get("status") as string) || "active";
      imageFiles = formData.getAll("images") as File[];
      deletedImageIdsStr = formData.get("deletedImageIds") as string;
    }

    // Support quick status toggling without requiring the other listing fields
    const isStatusOnlyUpdate = statusStr && !title && !description && !priceStr && !categoryIdStr;

    let updateFields: any = {};
    if (isStatusOnlyUpdate) {
      if (!["active", "sold", "inactive"].includes(statusStr)) {
      ...
      if (statusStr && !["active", "sold", "inactive"].includes(statusStr)) {
        return NextResponse.json({ error: "Invalid status." }, { status: 400 });
      }
    ...
    // Handle image deletions (only if not a quick status update)
    if (!isStatusOnlyUpdate && deletedImageIdsStr) {
    ...
    // Save and append new images (only if not a quick status update)
    if (!isStatusOnlyUpdate && imageFiles.length > 0) {
...
export async function DELETE(
  ...
  try {
    ...
    // Owner or Admin protection
    const isAdmin = isAdminEmail(session.user.email) || session.user.roleSlug === "admin";
    if (listing[0].sellerEmail.toLowerCase() !== session.user.email.toLowerCase() && !isAdmin) {
      return NextResponse.json({ error: "Forbidden: You do not own this listing." }, { status: 403 });
    }

    // Delete listing (this cascades to images, wishlist, cart items, etc.)
